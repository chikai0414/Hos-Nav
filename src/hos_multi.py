...
class N_Step_ReplayBuffer:
    def __init__(self, capacity, n_step = 5):
        self.capacity = capacity
        self.buffer = []
        self.gamma = 0.95
        self.n_step = n_step
    ...
    def sample(self, batch_size):
        ...
        for i in range(batch_size):
            end = random.randint(self.n_step, self.__len__() - 1)
            begin = end-self.n_step
            sum_reward = 0 # n_step rewards
            data = self.buffer[begin:end]
            state = data[0][0]
            action = data[0][1]
...
class SAC(object):
    ...
    def update_parameters(self, memory, batch_size):
        ...
        next_state_batch = np.reshape(next_state_batch,(batch_size,state_dim))
...
max_episodes  = 100 ## 10001
max_steps   = 1000 ###10000
rewards     = []
batch_size  = 256

action_dim = 2
state_dim  = 362  #360+4+6
#hidden_dim = 500
...
replay_buffer = N_Step_ReplayBuffer(200000,5)
#agent.load_models(120)
...
if __name__ == '__main__':
    ...
    if name == "/sac_1":
        env = Env(model = 1)
        pub_s_esp = rospy.Publisher('s_esp/agent_1', Float32, queue_size=5)
        pub_result = rospy.Publisher('result/agent_1', Float32, queue_size=5)


    elif name == "/sac_2":
        #exit()
        env = Env(model = 2)
        pub_s_esp = rospy.Publisher('s_esp/agent_2', Float32, queue_size=5)
        pub_result = rospy.Publisher('result/agent_2', Float32, queue_size=5)

    agent.load_models(600)
    before_training = 1 ##3
    ...
    for ep in range(max_episodes):
        ...
        for step in range(max_steps):
            ...
            print(name + 'state',state[-2:])
            print(name + 'action', unnorm_action,'r',reward)
            past_action = copy.deepcopy(action)

            rewards_current_episode += reward
            next_state = np.float32(next_state)
            #print('memory len' + str(len(replay_buffer)))
