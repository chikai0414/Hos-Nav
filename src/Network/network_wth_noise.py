...
class QNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim):
        ...
        self.linear2_q1 = nn.Linear(32 + 2 + 2 + 1, hidden_dim)
        ...
        self.linear2_q2 = nn.Linear(32 + 2 + 2 + 1 , hidden_dim)
    ...
    def forward(self, state, action):
        t = state.reshape(-1).__len__()/363
        state = torch.reshape(state,(t,1,363))
        goal = state[:,:,360:362]
        goal = torch.reshape(goal,(t,2))
        noise = state[:,:,362]
        noise = torch.reshape(noise,(t,1))
        state = state[:,:,:360]
        ...
        x1 = torch.cat([x1,action,goal,noise], 1)
        ...
        x2 = torch.cat([x2,action,goal,noise], 1)
...
class PolicyNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, log_std_min=-20, log_std_max=2):
        ...
        self.linear2 = nn.Linear(32 + 2 + 1, hidden_dim)
